...
const { FiRefreshCw, FiAlertTriangle, FiShield, FiActivity, FiLogIn } = FiIcons;

const Header = ({ totalAlerts, criticalAlerts, lastUpdated, onRefresh, loading, user }) => {
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
              <div className="flex gap-4">
                <div className={`flex items-center gap-2 px-3 py-2 ${
                  totalAlerts > 0 ? 'bg-blue-50' : 'bg-green-50'
                } rounded-lg`}>
                  <SafeIcon 
                    icon={FiShield} 
                    className={totalAlerts > 0 ? 'text-blue-600' : 'text-green-600'} 
                  />
                  <span className={`text-sm font-medium ${
                    totalAlerts > 0 ? 'text-blue-800' : 'text-green-800'
                  }`}>
                    {totalAlerts > 0 
