...
// Helper function to get real-time service health (would require authentication)
export const getAuthenticatedServiceHealth = async (accessToken) => {
  try {
    const headers = {
    ...
    // Microsoft Graph API endpoints for service health
    const endpoints = [
      `${API_BASE}/admin/serviceAnnouncement/healthOverviews`,
      `${API_BASE}/admin/serviceAnnouncement/issues`
    ];
    ...
    return processServiceHealthData(responses);
  } catch (error) {
    console.error('Error fetching authenticated service health:', error);
    throw error;
  }
};

const processServiceHealthData = (responses) => {
  // Process and normalize the API responses
  const [healthOverviews, issues] = responses;
  
  // Transform the data into our expected format
  return healthOverviews.data.value.map(service => ({
    id: service.service,
    name: getServiceDisplayName(service.service),
    status: mapHealthStatus(service.status),
    alerts: issues.data.value
      .filter(issue => issue.service === service.service)
      .map(issue => ({
        id: issue.id,
        title: issue.title,
        severity: mapSeverity(issue.classification),
        status: mapIssueStatus(issue.status),
        impact: issue.impactDescription,
        startTime: new Date(issue.startDateTime),
        lastUpdated: new Date(issue.lastModifiedDateTime),
        affectedServices: issue.feature ? [issue.feature] : [],
        region: issue.userImpact || 'Global'
      }))
  }));
};
...
const mapSeverity = (classification) => {
  const severityMap = {
    'Advisory': 'low',
    'Incident': 'medium',
    'Emergency': 'high'
...
const mapIssueStatus = (status) => {
  const statusMap = {
    ...
    'InvestigationSuspended': 'monitoring',
    'ServiceDegradation': 'investigating'
  };
